    ...
    <Card class="w-full max-w-md bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-slate-200 dark:border-slate-700" padding="lg">
      <template #header>
        <div class="text-center">
          <div class="w-16 h-16 bg-slate-100 dark:bg-slate-700 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <CubeIcon class="h-8 w-8 text-slate-600 dark:text-slate-300" />
          </div>
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Blockchain Computing</h2>
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-2">Login to Distributed Blockchain Platform</p>
        </div>
      </template>
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        <Input
          ...
          label="Username"
          placeholder="Enter username or try: blockchain_demo"
          required
        />
        ...
        <div class="text-xs text-gray-500 dark:text-gray-400 text-center">
          Demo credentials: <code class="bg-gray-100 dark:bg-gray-800 px-1 rounded">blockchain_demo</code> / <code class="bg-gray-100 dark:bg-gray-800 px-1 rounded">demo123</code>
        </div>
...
const handleLogin = async () => {
  ...
  try {
    const result = await authStore.login(credentials.value, 'blockchain')
    if (result.success) {
...
const quickLogin = async () => {
  ...
  try {
    const demoCredentials = {
      username: 'blockchain_demo',
      password: 'demo123'
    }
    
    const result = await authStore.quickLogin(demoCredentials, 'blockchain')
    if (result.success) {
      router.push('/blockchain/dashboard')
    }
  } catch (error) {
