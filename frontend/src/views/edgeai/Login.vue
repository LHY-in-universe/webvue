    ...
    <Card class="w-full max-w-md bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm border border-slate-200 dark:border-slate-700" padding="lg">
      <template #header>
        <div class="text-center">
          ...
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-2">Login to Edge AI Intelligence Platform</p>
        </div>
      </template>
      ...
      <form @submit.prevent="handleLogin" class="space-y-6">
        <Input
          ...
          label="Username"
          placeholder="Enter username or try: edgeai_demo"
          required
        />
...
const handleLogin = async () => {
  ...
  try {
    const result = await authStore.login(credentials.value, 'edgeai')
    if (result.success) {
